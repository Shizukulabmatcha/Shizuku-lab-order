...
const DEMO_MENU = [
  { id: "duo", category: "Bundle", name: "Shizuku Duo", description: "Signature Matcha Latte + Signature Dark Roasted Hojicha Latte", price: 10.5, image_url: "images/matcha-latte.jpg" },
  { id: "sig-matcha", category: "Signature", name: "Signature Matcha Latte", description: "Fresh whisked First Harvest matcha with oat milk", price: 5.9, image_url: "images/matcha-latte.jpg" },
  { id: "sig-hojicha", category: "Signature", name: "Signature Hojicha Latte", description: "Fresh whisked First Harvest dark roasted hojicha with oat milk", price: 5.9, image_url: "images/hojicha-latte.jpg" },
  { id: "craft", category: "Craft your own", name: "Matcha Lab", description: "Pick your grade, milk and sweetness", price: 5.9, image_url: "images/matcha-lab.jpg" },
];
...
async function init() {
  ...
  if (IS_CONFIGURED) {
    const { data, error } = await db.from("product").select("*").eq("is_available", true).order("category");
    state.menu = error || !data ? [] : data;
    if (error) console.error("Could not load products:", error.message);
  } else {
  ...
  return `
    ...
    <div class="menu-list">
      ${items.length === 0 ? `<div class="empty">No items yet — add some from the shop dashboard.</div>` : items.map((item) => `
        <div class="item-card">
          <img class="item-thumb" src="${item.image_url || "images/matcha-lab.jpg"}" alt="${item.name}">
          <div class="item-info">
  ...
  return `
    ...
    <div class="screen">
      ...
      ${lines.length === 0 ? `<div class="empty">Your cart is empty — the whisk is waiting.</div>` : lines.map((l) => `
        <div class="item-card">
          <img class="item-thumb" src="${l.item.image_url || "images/matcha-lab.jpg"}" alt="${l.item.name}">
          <div class="item-info">
