...
-- This removes only the old lookup function. It does not delete any orders.
...
as $$
  ...
  where
    (nullif(trim(coalesce(p_order_number,'')), '') is not null and upper(o.order_number) = upper(trim(p_order_number)))
    or
    (nullif(regexp_replace(coalesce(p_phone,''),'[^0-9]','','g'), '') is not null and regexp_replace(coalesce(o.customer_phone,''),'[^0-9]','','g') in (
