(() => {
  ...
  async function setVisibility(id,value) {
    ...
    if (!productionClient) {
      ...
      return alert("Hosted Slow Studio means the online page at slow-studio-demo.vercel.app/slow-studio. A file:// page is only a local preview and cannot update Supabase or a live website.");
    }
  ...
  async function createHbbAccount(event) {
    ...
    if (!productionClient) return alert("Hosted Slow Studio means the online page at slow-studio-demo.vercel.app/slow-studio. Open that online page before adding a real HBB account; a file:// preview cannot connect to Supabase.");
    const { data:{ user } } = await productionClient.auth.getUser();
