...
const ADMIN_WORKSPACE_MARKET = new URLSearchParams(window.location.search).get("market") === "MY" ? "MY" : "SG";
const ADMIN_CUSTOMER_SHOP_URL = ADMIN_WORKSPACE_MARKET === "MY" ? "/shop/shizuku-lab-my" : "/shop/shizuku-lab-sg";
...
function renderMarketingTab() { return renderMarketingContactManager() + renderMarketingCampaigns(); }
...
function renderAnalyticsReportTab() {
  ...
  const periodProfit = periodRevenue - periodFoodCost;
  ...
  return `
    ...
    <div class="analytics-kpi-grid">
      ...
      <section class="analytics-kpi"><span>${periodLabel} gross profit</span><strong>${money(periodProfit)}</strong><small>Sales minus food cost</small></section>
      <section class="analytics-kpi"><span>${periodLabel} margin</span><strong>${periodMargin.toFixed(1)}%</strong><small>${money(periodProfit)} gross profit</small></section>
    ...
    <div class="analytics-report-grid analytics-report-grid-secondary">
      <section class="dashboard-card"><div class="dashboard-card-head"><div><h2>Cost & margin</h2><span>Direct food cost only</span></div></div><div class="analytics-breakdown"><div><span>${periodLabel} sales</span><b>${money(periodRevenue)}</b></div><div><span>${periodLabel} food cost</span><b>${money(periodFoodCost)}</b></div><div><span>${periodLabel} gross profit</span><b>${money(periodProfit)}</b></div><div><span>${periodLabel} margin</span><b>${periodMargin.toFixed(1)}%</b></div></div>${stats.missingRecipeProducts.length ? `<div class="ref-note"><b>${stats.missingRecipeProducts.length} product cost warning(s)</b><br>${stats.missingRecipeProducts.map((product) => escapeHtml(product.name)).join(", ")}</div>` : ""}</section>
...
function renderEditOverlay() {
  ...
  <div class="overlay">
    <div class="overlay-card" style="max-height:80vh;overflow-y:auto;">
      ...
      <div class="divider"></div><div class="display" style="font-size:18px;margin-bottom:10px">Malaysia · MYR</div>
...
function render() {
  ...
  const navGroups = [
    ...
    { key:"business", label:"Business tools", icon:"◇", tabs:[["wholesale","◇","Wholesale / B2B"],["inspiration","✎","Inspiration"],["team","♙","Team & activity"],["malaysia","MY","Malaysia workspace"]] },
  ];
  ...
  const page = astate.tab === "dashboard" ? renderDashboardTab() : `
    <div class="admin-top"><div><div class="admin-eyebrow">Shizuku Lab · ${DASHBOARD_MARKET === "MY" ? "Malaysia" : "Singapore"}</div><h1 class="tab-page-title">${tabTitle[astate.tab] || "Dashboard"}</h1><p class="tab-page-subtitle">${tabSubtitle[astate.tab] || ""}</p></div><a class="open-shop" href="${ADMIN_CUSTOMER_SHOP_URL}">Open customer shop ↗</a></div>
  ...
  app.innerHTML = `
    ...
    <div class="shop-admin theme-${escapeHtml(astate.settingsDraft?.admin_theme || astate.settingsDraft?.system_theme || "zen")} ${(astate.settings?.admin_mobile_nav_position || "left") === "top" ? "mobile-nav-top" : "mobile-nav-left"} ${astate.navCollapsed ? "nav-collapsed" : ""}" style="--admin-primary:${escapeHtml(astate.settingsDraft?.admin_theme_primary || '#4B5D3A')};--admin-bg:${escapeHtml(astate.settingsDraft?.admin_theme_background || '#F3EEE3')};--admin-card:${escapeHtml(astate.settingsDraft?.admin_theme_card || '#FFFFFF')};--admin-text:${escapeHtml(astate.settingsDraft?.admin_theme_text || '#2A2A22')};--admin-on-primary:${escapeHtml(astate.settingsDraft?.admin_theme_background || '#F3EEE3')};--admin-soft:color-mix(in srgb,${escapeHtml(astate.settingsDraft?.admin_theme_primary || '#4B5D3A')} 10%,${escapeHtml(astate.settingsDraft?.admin_theme_card || '#FFFFFF')});--admin-line:color-mix(in srgb,${escapeHtml(astate.settingsDraft?.admin_theme_text || '#2A2A22')} 18%,transparent);--admin-muted:color-mix(in srgb,${escapeHtml(astate.settingsDraft?.admin_theme_text || '#2A2A22')} 66%,transparent);--admin-radius:${(astate.settingsDraft?.admin_theme || 'zen') === 'editorial' ? '0px' : (astate.settingsDraft?.admin_theme || 'zen') === 'retro' ? '7px' : (astate.settingsDraft?.admin_theme || 'zen') === 'threed' ? '24px' : '18px'};--admin-card-shadow:${(astate.settingsDraft?.admin_theme || 'zen') === 'retro' ? '4px 4px 0 var(--admin-text)' : (astate.settingsDraft?.admin_theme || 'zen') === 'threed' ? '0 10px 0 color-mix(in srgb,var(--admin-primary) 18%,transparent),0 17px 28px rgba(45,38,75,.11)' : '0 8px 24px rgba(42,42,34,.06)'};--admin-shadow:0 10px 24px color-mix(in srgb,var(--admin-primary) 25%,transparent);">
      <aside class="admin-side"><button class="admin-collapse-toggle" onclick="toggleAdminNav()" title="${astate.navCollapsed ? "Expand menu" : "Collapse menu"}" aria-label="${astate.navCollapsed ? "Expand menu" : "Collapse menu"}">${astate.navCollapsed ? "›" : "‹"}</button><div class="admin-logo">${(astate.settings && escapeHtml(astate.settings.store_name)) || "Shizuku Lab"}</div><div class="admin-caption">SLOW STUDIO · SHOP ADMIN</div><div class="admin-nav-label">MAIN MENU</div><nav class="admin-nav"><button class="admin-nav-root ${astate.tab === "dashboard" ? "active" : ""}" onclick="setTab('dashboard')"><span class="nav-icon">⌂</span><span class="nav-text">Home</span></button>${navGroupHtml}<button class="admin-nav-root ${astate.tab === "analytics" ? "active" : ""}" onclick="setTab('analytics')"><span class="nav-icon">▥</span><span class="nav-text">Analytics Report</span></button></nav><div class="admin-side-bottom"><a class="slow-studio-link" href="/slow-studio">Slow Studio workspace →</a><button class="link-btn" onclick="logoutAdmin()" aria-label="Sign out"><span class="signout-icon">↪</span> <span class="signout-label">Sign out</span></button></div></aside>
